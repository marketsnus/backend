...
def add_payment_info():
    ...
    try:
        ...
        payment = PaymentInfo(
            ...
            active=True  # Новый платеж будет активным, но не деактивирует другие
        )
...
def toggle_payment_status(payment_id):
    ...
    try:
        payment = PaymentInfo.query.get_or_404(payment_id)
        payment.active = not payment.active  # Просто переключаем статус без деактивации других
        db.session.commit()
...
def get_payment_api():
    ...
    try:
        # Получаем все активные платежные данные
